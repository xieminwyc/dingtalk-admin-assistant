...
const DEFAULT_VISUAL_RESPONSE_TIMEOUT_MS = 45_000;
...
export function createResponseGenerator(
  ...
  return {
    async generate(generatorInput) {
      try {
        const imageUrls = normalizeImageUrls(generatorInput);
        console.info(
          formatResponseLog(
        ...
        const currentDateInfo = `当前日期：${now.getFullYear()}年${now.getMonth() + 1}月${now.getDate()}日，星期${weekdays[now.getDay()]}`;
          ...
          const timeoutMs = imageUrls.length > 0
            ? DEFAULT_VISUAL_RESPONSE_TIMEOUT_MS
        ...
        console.warn(
          formatResponseLog(
            `response mode=${generatorInput.resolution.kind} query="${generatorInput.query}" failed reason="${reason}"`,
          ),
        );
        return null;
      }

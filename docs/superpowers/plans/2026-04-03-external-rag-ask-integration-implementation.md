...
- 实际新增/修改文件：
  ...
  - `src/modules/assistant/create-assistant-runtime.test.ts`
...
- 同步 `/ask` 当前实际使用 `excludeImageData: false`，图片不是“只接不显”，而是正式透传字段
- `createAssistantRuntime` 不仅暴露 `externalKnowledge.ask()`，也暴露了 `askStream()` 与会话映射 helper，供首页流式路由复用
- 首页流式链路在缺图时会回退到同步 `/ask.pics`，因此这份同步接口实施计划已经成为首页图片能力的一部分，而不只是知识命中主链路

## 已执行验证

- `npm test -- src/modules/assistant/create-assistant-runtime.test.ts src/app/api/dingtalk/webhook/stream/route.test.ts`
